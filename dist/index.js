...
server.tool("generate", "Generate UUIDs (v4), random hex strings, or nanoid-style IDs", {
...
server.tool("encode_decode", "Encode or decode text: Base64, URL encoding, HTML entities, hex", {
    ...
    try {
        if (format === "base64") {
            ...
            if (direction === "encode") {
                ...
                result = text.replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">")
                    .replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&#x([0-9a-fA-F]+);/g, (_, hex) => String.fromCodePoint(parseInt(hex, 16)))
...
// Tool 3: Hash text
server.tool("hash", "Generate hash digests: MD5, SHA-1, SHA-256, SHA-512", {
...
server.tool("jwt_decode", "Decode a JWT token and display header, payload, and expiration status (no signature verification)", {
    token: z.string().describe("The JWT token to decode"),
}, async ({ token }) => {
    try {
        const parts = token.split(".");
        if (parts.length !== 3)
            return { content: [{ type: "text", text: "Error: Invalid JWT format (expected 3 parts)" }] };
...
server.tool("json_utils", "JSON utilities: format/prettify, minify, validate, diff two JSON objects, or extract a path", {
    ...
    try {
        ...
        if (action === "diff") {
            if (!json2)
                return { content: [{ type: "text", text: "Error: json2 required for diff" }] };
            ...
            function compare(obj1, obj2, prefix) {
                ...
                for (const key of keys) {
                    ...
                    if (!(key in obj1)) {
                        diffs.push(`+ ${p}: ${JSON.stringify(obj2[key])}`);
                    }
                    else if (!(key in obj2)) {
                        diffs.push(`- ${p}: ${JSON.stringify(obj1[key])}`);
                    }
        ...
        if (action === "extract") {
            if (!path)
                return { content: [{ type: "text", text: "Error: path required for extract" }] };
...
// Tool 6: Regex tester
server.tool("regex_test", "Test a regex pattern against text: shows all matches, groups, and indices", {
